...
#    Updated: 2020/01/19 20:22:12 by Kay Zhou         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def load_all_tweetid(sess):
    print("load tweets id ...")
    set_tweets = set()
    tweets = sess.query(Tweet.tweet_id).yield_per(10000)
    for t in tweets:
        set_tweets.add(t[0])
        # print(t[0])
    return set_tweets 
...
def to_all_tweetid(sess):
    print("load tweets id ...")
    tweets = sess.query(Tweet.tweet_id).yield_per(10000)
    with open("new_disk/all_tweets_20200105.csv", "w") as f:
        for t in tweets:
            f.write(t[0])
...
def read_all_raw_data():
    ...
    # set_tweets = load_all_tweetid(sess)
    set_tweets = set()
    for in_name in os.listdir("new_disk/raw_tweets"):
        print(in_name, "start ...")
        in_name = "new_disk/raw_tweets/" + in_name
        cnt = 0
...
def tweets_to_db_v2(sess, start, end, clear=False):
    ...
    from DLModel.my_model import Camp_Classifier
    Lebron = Camp_Classifier()

    X = []
    tweets_data = []

    for d, dt in read_all_raw_data():
        tweet_id = d["id"]
        uid = d["user"]["id"]
        _sou = get_source_text(d["source"])
...
def db_to_stat_predict_v2(sess, start, end):
    ...
    out_file = open("data/stat_v2.json", "w")
...
def save_user_snapshot(start, end, w=14, prob=0.68):
    ...
    for dt in pendulum.Period(start, end):
        ...
        for now in pendulum.Period(dt, _end):
            _u = json.load(open(f"disk/users_v2/{now.to_date_string()}-{prob}.txt"))
            users = union_users(users, _u)

        json.dump(users, open(
            f"disk/users-{w}days_v2/{_end.to_date_string()}-{prob}.txt", "w"))
...
def predict_user_snapshot(win=7):
    ...
    end = pendulum.datetime(2019, 10, 28, tz="UTC")
    _period = pendulum.Period(start, end)
...
def get_session():
    engine = create_engine(
        "sqlite:////home/alex/kayzhou/Argentina_election/data/election_fastaiV2.db")
...
def tweets_day_to_csv(sess, dt):
    """
    每日tweets导入到csv
    """
    tweets = get_tweets_day(sess, dt)
    with open(f"data/csv/{dt.to_date_string()}.csv", "w") as f:
        for t in tweets:
            f.write(f"{t.tweet_id},{t.user_id},{t.proM}\n")
...
if __name__ == "__main__":
    init_db()
    sess = get_session()
    start = pendulum.datetime(2019, 2, 20, tz="UTC") # include this date
    end = pendulum.datetime(2019, 10, 28, tz="UTC") # not include this date
    print(f"{start} <= run < {end} ")
    tweets_to_db_v3(sess)

    # 获取每日tweets
    # for dt in pendulum.Period(start, end):
        # print(dt)
        # tweets_day_to_csv(sess, dt)
